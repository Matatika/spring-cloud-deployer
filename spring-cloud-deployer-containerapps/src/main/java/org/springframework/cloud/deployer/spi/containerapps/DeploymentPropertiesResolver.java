...
import org.springframework.cloud.deployer.spi.containerapps.ContainerAppsDeployerProperties.ConfigMapKeyRef;
import org.springframework.cloud.deployer.spi.containerapps.ContainerAppsDeployerProperties.Identity;
import org.springframework.cloud.deployer.spi.containerapps.ContainerAppsDeployerProperties.InitContainer;
import org.springframework.cloud.deployer.spi.containerapps.ContainerAppsDeployerProperties.JobConfiguration;
import org.springframework.cloud.deployer.spi.containerapps.ContainerAppsDeployerProperties.RegistryCredentials;
import org.springframework.cloud.deployer.spi.containerapps.ContainerAppsDeployerProperties.Secret;
import org.springframework.cloud.deployer.spi.containerapps.ContainerAppsDeployerProperties.SecretKeyRef;
import org.springframework.cloud.deployer.spi.containerapps.probe.ProbeType;
 ...
 * deployment properties for the given Container Apps deployer properties.
 *
...
class DeploymentPropertiesResolver {
	...
	private ContainerAppsDeployerProperties properties;

	DeploymentPropertiesResolver(String propertyPrefix, ContainerAppsDeployerProperties properties) {
	...
	/**
	 * 
	 * Identity properties define the identity of the Container App Job created for a Task:
	 *
	 * Identity can be specified as deployer properties as well as app deployment properties.
	 * Deployment properties override deployer properties.
	 * 
	 * <code>
	 *     spring.cloud.deployer.containerapps.identity.type=SystemAssigned
	 * </code>
	 * 
	 * <code>
	 *     spring.cloud.deployer.containerapps.accounts.default.identity.type=SystemAssigned
	 * </code>
	 * 
	 * @param deployerProperties
	 * @return
	 */
	Identity getIdentity(Map<String, String> deployerProperties) {
		String identityType = PropertyParserUtils.getDeploymentPropertyValue(deployerProperties,
				this.propertyPrefix + ".identity.type", "");

		if (StringUtils.hasText(identityType)) {
			Identity identity = new Identity();
			identity.setType(IdentityType.valueOf(identityType));
			return identity;
		}
	...
	/**
	 * 
	 * JobConfiguration properties define the configuration for the Container App Job created for a Task:
	 *
	 * JobConfiguration can be specified as deployer properties as well as app deployment properties.
	 * Deployment properties override deployer properties.
	 * 
	 * <code>
	 *     spring.cloud.deployer.containerapps.jobConfiguration={ registries: [{ server: 'matatika.azurecr.io', identity: 'system' }] }
	 * </code>
	 * 
	 * <code>
	 *     spring.cloud.deployer.containerapps.accounts.default.jobConfiguration={ registries: [{ server: 'matatika.azurecr.io', identity: 'system' }] }
	 * </code>
	 * 
	 * @param deployerProperties
	 * @return
	 */
	JobConfiguration getJobConfiguration(Map<String, String> deployerPropertiesMap) {
		JobConfiguration jobConfiguration = new JobConfiguration();

		/*
		 * bind the string deployer properties.
		 * These properties cannot be bound directly by the Spring property Binder
		 */
		if (deployerPropertiesMap.get(this.propertyPrefix + ".jobConfiguration.secrets") == null
				&& properties.getSecrets() != null) {
			List<Secret> secrets = getListFromPropertyValue(properties.getSecrets(), Secret.class);
			properties.getJobConfiguration().setSecrets(secrets);
		}
		if (deployerPropertiesMap.get(this.propertyPrefix + ".jobConfiguration.registries") == null
				&& properties.getRegistries() != null) {
			List<RegistryCredentials> registries = getListFromPropertyValue(properties.getRegistries(), RegistryCredentials.class);
			properties.getJobConfiguration().setRegistries(registries);
		}

		ContainerAppsDeployerProperties deployerProperties = bindProperties(deployerPropertiesMap,
				this.propertyPrefix + ".jobConfiguration", "jobConfiguration");
		...
		deployerProperties
			.getJobConfiguration()
			.getSecrets()
			.forEach(s -> jobConfiguration.getSecrets().add(new Secret(
					s.getName(),
					s.getValue(),
					s.getIdentity(),
					s.getKeyVaultUrl())));
		...
		this.properties
			...
			.getSecrets()
				.stream()
				.filter(s -> {
					return jobConfiguration.getSecrets().stream()
		...
		deployerProperties
			.getJobConfiguration()
			.getRegistries()
			.forEach(registry -> jobConfiguration.getRegistries().add(new RegistryCredentials(
					registry.getServer(),
					registry.getUsername(),
					registry.getPasswordSecretRef(),
		...
		this.properties
			...
			.getRegistries()
				.stream()
				.filter(registry -> {
					return jobConfiguration.getRegistries().stream()
							.noneMatch(existing -> existing.getServer().equals(registry.getServer()));
				})
				.collect(Collectors.toList())
				.forEach(registry -> jobConfiguration.getRegistries().add(new RegistryCredentials(
					registry.getServer(),
	...
	List<Toleration> getTolerations(Map<String, String> kubernetesDeployerProperties) {
		...
		ContainerAppsDeployerProperties deployerProperties = bindProperties(kubernetesDeployerProperties,
	...
	List<Volume> getVolumes(Map<String, String> kubernetesDeployerProperties) {
		...
		volumes.addAll(properties.getVolumes().stream()
				.filter(volume -> volumes.stream()
						.noneMatch(existingVolume -> existingVolume.name().equals(volume.name())))
				.collect(Collectors.toList()));
	...
	/**
	 * Get the location setting for the deployment request.
	 *
	 * @param deployerProperties the kubernetes deployment properties map
	 * @return location is the Azure region
	 */
	String getLocation(Map<String, String> kubernetesDeployerProperties) {
		String locationOverride = PropertyParserUtils.getDeploymentPropertyValue(kubernetesDeployerProperties,
						this.propertyPrefix + ".location");
		String location;

		if (!StringUtils.hasText(locationOverride)) {
	...
	/**
	 * Get the environment setting for the deployment request.
	 *
	 * @param deployerProperties the kubernetes deployment properties map
	 * @return environment is the name of the Container Apps environment
	 */
	String getEnvironment(Map<String, String> kubernetesDeployerProperties) {
		String environmentOverride = PropertyParserUtils.getDeploymentPropertyValue(kubernetesDeployerProperties,
						this.propertyPrefix + ".environment");
		String environment;
	...
	private PodSecurityContext buildPodSecurityContext(ContainerAppsDeployerProperties deployerProperties) {
	...
	private SecurityContext buildContainerSecurityContext(ContainerAppsDeployerProperties deployerProperties) {
	...
	private Container containerFromProps(InitContainer initContainerProps) {
		return new Container()
				...
				.withVolumeMounts(Optional.ofNullable(initContainerProps.getVolumeMounts()).orElse(Collections.emptyList()));
	}

	private List<EnvironmentVar>  toEnvironmentVariables(String[] environmentVariables) {
	...
	List<Container> getAdditionalContainers(Map<String, String> deploymentProperties) {
		...
		ContainerAppsDeployerProperties deployerProperties = bindProperties(deploymentProperties,
		...
		if (this.properties.getAdditionalContainers() != null) {
			this.properties.getAdditionalContainers().stream()
					.filter(container -> containers.stream().noneMatch(existing -> existing.name().equals(container.getName())))
		...
		if (StringUtils.hasText(updatedLabels)) {
			...
			for (String label : deploymentLabel) {
				...
				Assert.isTrue(labelPair.length == 2,
						String.format("Invalid label format, expected 'labelKey:labelValue', got: '%s'", (Object[]) labelPair));
	...
	RestartPolicy getRestartPolicy(Map<String, String> deployerProperties) {
		String restartPolicy = PropertyParserUtils.getDeploymentPropertyValue(deployerProperties,
	...
	/**
	 * Binds the YAML formatted value of a deployment property to a {@link ContainerAppsDeployerProperties} instance.
	 *
	 ...
	 * @return a {@link ContainerAppsDeployerProperties} with the bound property data
	 */
	private static ContainerAppsDeployerProperties bindProperties(Map<String, String> deployerPropertiesMap,
			String propertyKey, String yamlLabel) {
		String deploymentPropertyValue = PropertyParserUtils.getDeploymentPropertyValue(deployerPropertiesMap, propertyKey);
		...
		if (StringUtils.hasText(deploymentPropertyValue)) {
			try {
				...
				deployerProperties = new Binder(source)
						.bind("", Bindable.of(ContainerAppsDeployerProperties.class)).get();
			} catch (Exception e) {
	...
	/*
	 * Get a List<YourType> from the supplied property.
	 * The value should be a json array. e.g. [ { key: 'value' } ]
	 * Where YourType has a java property getKey/setKey
	 */
	<T> List<T> getListFromPropertyValue(String deploymentPropertyValue, Class<T> clazz) {
		assert(deploymentPropertyValue != null);
		List<T> result = new ArrayList<>();
		if (StringUtils.hasText(deploymentPropertyValue)) {
			try {
				YamlPropertiesFactoryBean properties = new YamlPropertiesFactoryBean();
				properties.setResources(new ByteArrayResource(deploymentPropertyValue.getBytes()));
				Properties yaml = properties.getObject();
				MapConfigurationPropertySource source = new MapConfigurationPropertySource(yaml);
				result = new Binder(source).bind("document", Bindable.listOf(clazz)).get();
			} catch (Exception e) {
				throw new IllegalArgumentException(
						String.format("Invalid binding property '%s'", deploymentPropertyValue), e);
			}
	...
	private List<VolumeMount> getVolumeMounts(String propertyValue) {
		...
		if (StringUtils.hasText(propertyValue)) {
			try {
				...
				Properties yaml = properties.getObject();
				MapConfigurationPropertySource source = new MapConfigurationPropertySource(yaml);
				ContainerAppsDeployerProperties deployerProperties = new Binder(source)
						.bind("", Bindable.of(ContainerAppsDeployerProperties.class)).get();
		...
		volumeMounts.addAll(this.properties.getVolumeMounts().stream().filter(volumeMount -> volumeMounts.stream()
				.noneMatch(existingVolumeMount -> existingVolumeMount.volumeName().equals(volumeMount.volumeName())))
				.collect(Collectors.toList()));
	 ...
	 */
	ContainerAppsDeployerProperties.Lifecycle getLifeCycle(Map<String,String> deploymentProperties) {
		ContainerAppsDeployerProperties.Lifecycle lifecycle = properties.getLifecycle();
	...
	private ContainerAppsDeployerProperties.Lifecycle.Hook lifecycleHook(String command) {
		ContainerAppsDeployerProperties.Lifecycle.Hook hook = new ContainerAppsDeployerProperties.Lifecycle.Hook();
		ContainerAppsDeployerProperties.Lifecycle.Exec exec = new ContainerAppsDeployerProperties.Lifecycle.Exec();
	...
	List<EnvironmentVar> getConfigMapKeyRefs(Map<String, String> deploymentProperties) {
		List<EnvironmentVar> configMapKeyRefs = new ArrayList<>();
		ContainerAppsDeployerProperties deployerProperties = bindProperties(deploymentProperties,
		...
		properties.getConfigMapKeyRefs().stream()
				.filter(configMapKeyRef -> configMapKeyRefs.stream()
						.noneMatch(existing -> existing.name().equals(configMapKeyRef.getEnvVarName())))
				.collect(Collectors.toList())
	...
	private EnvironmentVar buildConfigMapKeyRefEnvVar(ConfigMapKeyRef configMapKeyRef) {
...
//		EnvironmentVar configMapKeyEnvRefVar = new EnvironmentVar()
//				.withName(configMapKeyRef.getEnvVarName())
//				...
//		return configMapKeyEnvRefVar;
		throw new RuntimeException("Not supported");
	}

	List<EnvironmentVar> getSecretKeyRefs(Map<String, String> deploymentProperties) {
		List<EnvironmentVar> secretKeyRefs = new ArrayList<>();
		...
		properties.getSecretKeyRefs().stream()
				.filter(secretKeyRef -> secretKeyRefs.stream()
						.noneMatch(existing -> existing.name().equals(secretKeyRef.getEnvVarName())))
				.collect(Collectors.toList())
	...
	private EnvironmentVar buildSecretKeyRefEnvVar(SecretKeyRef secretKeyRef) {
		EnvironmentVar secretKeyEnvRefVar = new EnvironmentVar()
				.withName(secretKeyRef.getEnvVarName())
				.withSecretRef(secretKeyRef.getSecretName());
